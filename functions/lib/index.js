...
Responda APENAS com o JSON. NÃO explique o seu pensamento.`;
    const response = await ai.generate({
        ...
        output: { schema: schemas_js_1.bureaucracySchema }
    });
    if (!response.output) {
...
exports.thematicAgentFlow = ai.defineFlow({
...
exports.verificationAgentFlow = ai.defineFlow({
    ...
    inputSchema: zod_1.z.object({
        osc: schemas_js_1.ngoProfileSchema,
        editalText: zod_1.z.string().describe("Texto bruto completo do edital"),
    }),
    outputSchema: schemas_js_1.verificationResultSchema,
}, async (input) => {
    const currentDate = new Date().toISOString().split('T')[0];
...
Responda APENAS com o JSON. Nenhuma explicação adicional.`;
    const response = await ai.generate({
        ...
        output: { schema: schemas_js_1.verificationResultSchema }
    });
    if (!response.output) {
...
exports.verifyMatchConstraints = (0, https_1.onCall)({
    ...
    if (!matchId) {
        throw new https_1.HttpsError('invalid-argument', 'O parâmetro matchId é obrigatório.');
    }
    ...
    try {
        ...
        if (!oscId || !editalId) {
            throw new https_1.HttpsError('failed-precondition', 'O match não possui oscId ou editalId válidos.');
        }
        ...
        if (!oscDoc.exists || !editalDoc.exists) {
            throw new https_1.HttpsError('failed-precondition', 'OSC ou Edital não encontrados no banco de dados.');
        }
        const oscData = oscDoc.data();
        const editalData = editalDoc.data();
        const rawText = editalData.rawText || '';
        if (!rawText) {
            throw new https_1.HttpsError('failed-precondition', 'O texto bruto (rawText) do edital não está disponível para verificação.');
        }
        ...
        if (!oscParseResult.success) {
            throw new https_1.HttpsError('internal', 'Dados da OSC inválidos para verificação.');
        }
        // Run the agent flow
...
async function processMatchEvaluation(oscId, editalId, forceRecalculate = false) {
    ...
    if (!shouldRecalculate && existingMatchData) {
        ...
        if (!forceRecalculate && shouldRecalculate && ((typeof existingMatchData.status === 'string' && existingMatchData.status.includes('Inelegível')) ||
    ...
    if (similarityScore < dynamicThreshold) {
        ...
        if (isExpired) {
            ...
            try {
                const bureaucracyResult = await (0, exports.bureaucracyAgentFlow)({
                    osc: oscData,
                    edital: editalData
                });
                if (!bureaucracyResult.passesBureaucracy && !bureaucracyResult.rejectionReason?.toLowerCase().includes('pendente')) {
                    ...
                    matchResult = await (0, exports.thematicAgentFlow)({
                        osc: oscData,
                        edital: editalData,
                        oscId: oscId,
                        editalId: editalId
...
exports.agenticSearchWorker = (0, tasks_1.onTaskDispatched)({
    ...
    try {
        ...
        const rawOscData = oscDoc.data() || {};
        const hasDescriptiveText = Boolean(rawOscData.mission ||
        ...
        const parseResult = schemas_js_1.ngoProfileSchema.safeParse(enrichedOscData);
...
exports.triggerBulkInternalMatch = (0, https_1.onCall)({
    ...
    try {
        ...
        for (const osc of oscs) {
            ...
            if (!oscEmbedding) {
                const parseResult = schemas_js_1.ngoProfileSchema.safeParse(osc);
...
exports.recalculateDashboardStats = (0, https_1.onCall)({
    ...
    try {
        ...
        const isNotReprovado = (m) => !(m.actionState === 'Rejeitado' || (!m.actionState && m.eligibility === false));
        const hotLeadsMap = {};
