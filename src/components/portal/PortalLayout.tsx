...
const PortalLayoutInner: React.FC = () => {
  ...
  const oscPicker = oscOptions.length > 0 ? (
    <div className="px-3 py-2 mt-2">
      <label className="text-xs font-medium text-muted-foreground uppercase tracking-wider mb-1 block">OSC Ativa</label>
      <select
        value={activeOscId || ''}
        onChange={(e) => setActiveOscId(e.target.value || null)}
        className="w-full text-sm bg-muted/50 border border-border rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary truncate"
      >
          ...
          <div className="flex-1 px-4 max-w-[200px]">
             {oscOptions.length > 0 && (
               <select
                 value={activeOscId || ''}
                 onChange={(e) => setActiveOscId(e.target.value || null)}
                 className="w-full text-xs bg-muted/50 border border-border rounded-md px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-primary truncate"
               >
                 <option value="">Selecione uma OSC</option>
                 {oscOptions.map(osc => (
                   <option key={osc.id} value={osc.id}>{osc.name}</option>
